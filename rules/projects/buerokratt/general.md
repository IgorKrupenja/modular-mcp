...
- **Execution**: Save the script to a temp path and run with an absolute module path. Build it dynamically, e.g.:
  - `"/tmp/sync-upstream.sh" "$(pwd)/Training-Module"`
  - `"/tmp/sync-upstream.sh" "$(git rev-parse --show-toplevel)/Training-Module"`
...
- **Usage Examples**:
  - Sync and push: `"/tmp/sync-upstream.sh" "$(pwd)/Training-Module"` (with `required_permissions: ['all']`)
  - Sync without pushing: `"/tmp/sync-upstream.sh" "$(pwd)/Training-Module" no-push` (with `required_permissions: ['all']`)
